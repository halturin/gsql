...
static GSQLNavigationItem operators[] = {
...
void
nav_tree_refresh_operators (GSQLNavigation *navigation,
						 GtkTreeView *tv,
						 GtkTreeIter *iter)
{
	GSQL_TRACE_FUNC;
	...
	while (gsql_cursor_fetch (cursor, 1) > 0)
	{
		...
		if (var->value_type != G_TYPE_STRING)
		{
			...
			g_snprintf (key, 255, "%x%s%d%s",
				   session, owner, OPERATOR_ID, name);
		...
		gtk_tree_store_set (GTK_TREE_STORE(model), &child,
					GSQL_NAV_TREE_ID,			OPERATOR_ID,
					GSQL_NAV_TREE_OWNER,		owner,
					GSQL_NAV_TREE_IMAGE,		GSQLE_ORACLE_STOCK_OPERATORS,
					...
					GSQL_NAV_TREE_STRUCT, operators,
					GSQL_NAV_TREE_DETAILS, details,
					GSQL_NAV_TREE_NUM_ITEMS, G_N_ELEMENTS(operators),
					-1);
