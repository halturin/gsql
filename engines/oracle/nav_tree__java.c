...
static GSQLNavigationItem java[] = {
...
void
nav_tree_refresh_java (GSQLNavigation *navigation,
						 GtkTreeView *tv,
						 GtkTreeIter *iter)
{
	GSQL_TRACE_FUNC;
	...
	switch (id)
	{
		...
		default:
			GSQL_DEBUG ("Java. Unhandled type ID [%d]", id);
			return;		
	}
	...
	while (gsql_cursor_fetch (cursor, 1) > 0)
	{
		...
		gtk_tree_store_set (GTK_TREE_STORE(model), &child,
					...
					GSQL_NAV_TREE_STRUCT, java,
					GSQL_NAV_TREE_DETAILS, details,
					GSQL_NAV_TREE_NUM_ITEMS, G_N_ELEMENTS(java),
					-1);
