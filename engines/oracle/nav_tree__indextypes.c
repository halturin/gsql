...
static GSQLNavigationItem indextypes[] = {
...
void
nav_tree_refresh_indextypes (GSQLNavigation *navigation,
						 GtkTreeView *tv,
						 GtkTreeIter *iter)
{
	GSQL_TRACE_FUNC;
	...
	while (gsql_cursor_fetch (cursor, 1) > 0)
	{
		...
		if (var->value_type != G_TYPE_STRING)
		{
			...
			g_snprintf (key, 255, "%x%s%d%s",
				   session, owner, INDEX_TYPE_ID, name);
		...
		gtk_tree_store_set (GTK_TREE_STORE(model), &child,
					GSQL_NAV_TREE_ID,			INDEX_TYPE_ID,
					GSQL_NAV_TREE_OWNER,		owner,
					GSQL_NAV_TREE_IMAGE,		GSQLE_ORACLE_STOCK_INDEX_TYPES,
					...
					GSQL_NAV_TREE_STRUCT, indextypes,
					GSQL_NAV_TREE_DETAILS, details,
					GSQL_NAV_TREE_NUM_ITEMS, G_N_ELEMENTS(indextypes),
					-1);
