...
void
nav_tree_refresh_mviews (GSQLNavigation *navigation,
						 GtkTreeView *tv,
						 GtkTreeIter *iter)
{
	GSQL_TRACE_FUNC;
	...
	switch (id)
	{
		case TABLE_ID:
			...
			if (strncmp (owner, gsql_session_get_username (session), 64))
				sql = (gchar *) sql_oracle_mview_logs;
			
			stock = GSQLE_ORACLE_STOCK_MVIEW_LOGS;
