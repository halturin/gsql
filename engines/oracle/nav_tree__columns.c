						 ...
						 GtkTreeIter *iter, guint event)
{
	...
	/* get parent iter. if this iter are TABLE_ID then
	   we looking for table's indexes only */
	...
	if (strncmp (owner, gsql_session_get_username (session), 64))
	{
		switch (id)
		{
			case TABLE_ID:
			case VIEW_ID:
				cursor = gsql_cursor_new (session, (gchar *) sql_oracle_table_columns);
			...
			case INDEX_ID:
				cursor = gsql_cursor_new (session, (gchar *) sql_oracle_index_columns);
				state = gsql_cursor_open_with_bind (cursor,
													FALSE,
													GSQL_CURSOR_BIND_BY_POS,
													G_TYPE_STRING, owner,
			...
			case TRIGGER_ID:
				cursor = gsql_cursor_new (session, (gchar *) sql_oracle_trigger_columns);
				state = gsql_cursor_open_with_bind (cursor,
													FALSE,
													GSQL_CURSOR_BIND_BY_POS,
													G_TYPE_STRING, owner,
