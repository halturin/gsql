...
static GSQLNavigationItem tables[] = {
	{	COLUMNS_ID,
		GSQL_STOCK_COLUMNS,
		N_("Columns"), 
		sql_firebird_table_columns,	// sql
		NULL, 						// object_popup
		NULL,						// object_handler
	...
	{	INDEXES_ID,
		GSQL_STOCK_INDEXES,
		N_("Indexes"), 
		sql_firebird_table_indexes,	// sql
		NULL, 						// object_popup
		NULL,						// object_handler
	...
	{	TRIGGERS_ID,
		GSQL_STOCK_TRIGGERS,
		N_("Triggers"), 
		sql_firebird_table_triggers,// sql
		NULL, 						// object_popup
		NULL,						// object_handler
	...
	{	CONSTRAINTS_ID,
		...
		N_("Constraints"), 
		sql_firebird_table_constraints,	// sql
		NULL, 						// object_popup
		NULL,						// object_handler
...
//"  				<menuitem name=\"FirebirdTableCreate\" action=\"FirebirdActionTableCreate\" />	"
//"  				<menuitem name=\"FirebirdTableDrop\" action=\"FirebirdActionTableDrop\" />		"
//"  				<menuitem name=\"FirebirdTableAlter\" action=\"FirebirdActionTableAlter\" />		"
"  				<menuitem name=\"FirebirdTableBrowse\" action=\"FirebirdActionTableBrowse\" />	"
"	    </placeholder> "
"  </popup> "
"</ui>";
...
*/	
	{ "FirebirdActionTableBrowse", NULL, 
		N_("Browse data"), NULL, 
	...
	while (gsql_cursor_fetch (cursor, 1) > 0)
	{
		...
		if (var->value_type != G_TYPE_STRING)
		{
			...
			length = g_snprintf (NULL,0, "%x%s%d%s",
				   session, (gchar *) tmp->value, TABLE_ID, (gchar *) tmp->value);
...
nav_tree_tables_browse (gchar *name, gchar *owner)
{
	...
	cursor = gsql_cursor_new (session, 
							  (gchar *) sql_firebird_table_columns);
						 ...
						 GtkTreeIter *iter, guint event)
{
	...
	if (!gsql_navigation_get_action (navigation, "FirebirdActionTableBrowse"))
	{
		actions = gtk_action_group_new ("FirebirdPopupTableActions");
