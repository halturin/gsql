						 ...
						 GtkTreeIter *iter, guint event)
{
	...
	switch (id)
	{
		case TABLE_ID:
		case VIEW_ID:
			cursor = gsql_cursor_new (session, (gchar *) sql_firebird_table_columns);
		...
		case INDEX_ID:
			cursor = gsql_cursor_new (session, (gchar *) sql_firebird_index_columns);
			state = gsql_cursor_open_with_bind (cursor,
												FALSE,
												GSQL_CURSOR_BIND_BY_POS,
												G_TYPE_STRING, parent_name,
			...
			break;
		default:
			cursor = gsql_cursor_new (session, (gchar *) sql_firebird_all_columns);
			state = gsql_cursor_open_with_bind (cursor,
												FALSE,
												GSQL_CURSOR_BIND_BY_POS,
												G_TYPE_STRING, parent_name,
