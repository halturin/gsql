...
/*TODO 
	add into logon dialog:
 	-protocol changing;
 	-character set changing;
 	-role changing;
 	-collate changing;
...
#define ENGINE_ID    "engine_firebird"
#define ENGINE_NAME  "Firebird"
#define ENGINE_DESC  "GSQL module for Firebird supporting"
#define ENGINE_AUTHOR "Smyatkin Maxim"
...
engine_load (GSQLEngine *engine)
{
	...
	engine->file_logo = "firebird/firebird.png";
	engine->stock_logo = GSQLE_FIREBIRD_STOCK_FIREBIRD;
	...
	/* cursor */
	engine->cursor_open_with_bind = firebird_cursor_open_bind;
	engine->cursor_open = firebird_cursor_open;
	engine->cursor_fetch = firebird_cursor_fetch;
	

	add_pixmap_directory (PACKAGE_PIXMAPS_DIR "/firebird");
	engine_stock_init();
...
engine_logon_widget_new ()
{
	...
	gtk_table_attach (GTK_TABLE (table), port, 1, 2, 5, 6,
						(GtkAttachOptions) (GTK_EXPAND | GTK_FILL),
						(GtkAttachOptions) (0), 0, 0);
	gtk_spin_button_set_value (GTK_SPIN_BUTTON (port), 3050);
