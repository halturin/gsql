...
static GSQLNavigationItem functions[] = {
	{   ARGUMENTS_ID,
		GSQL_STOCK_ARGUMENTS,
		N_("Input"), 
		sql_firebird_fun_in_arguments,		// sql
		NULL, 						// object_popup
		NULL,						// object_handler
	...
	{   ARGUMENTS_ID,
		GSQL_STOCK_ARGUMENTS,
		N_("Output"), 
		sql_firebird_fun_out_arguments,		// sql
		NULL, 						// object_popup
		NULL,						// object_handler
		(GSQLNavigationHandler) nav_tree_refresh_arguments,	// expand_handler
		NULL,						// event_handler
...
void
nav_tree_refresh_functions (GSQLNavigation *navigation,
						 	GtkTreeView *tv,
						 	GtkTreeIter *iter)
{
	GSQL_TRACE_FUNC;
	...
	while (gsql_cursor_fetch (cursor, 1) > 0)
	{
		...
		if (var->value_type != G_TYPE_STRING)
		{
			...
			length = g_snprintf (NULL,0, "%x%s%d%s",
				   session, (gchar *) tmp->value, FUNCTION_ID, (gchar *) tmp->value);
		...
		gtk_tree_store_set (GTK_TREE_STORE(model), &child,
					GSQL_NAV_TREE_ID,			FUNCTION_ID,
					GSQL_NAV_TREE_OWNER,		owner,
					GSQL_NAV_TREE_IMAGE,		GSQL_STOCK_FUNCTIONS,
					...
					GSQL_NAV_TREE_STRUCT, functions,
					GSQL_NAV_TREE_DETAILS, details,
					GSQL_NAV_TREE_NUM_ITEMS,  G_N_ELEMENTS(functions),
					-1);
