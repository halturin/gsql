...
void
nav_tree_refresh_exceptions (GSQLNavigation *navigation,
						 	GtkTreeView *tv,
						 	GtkTreeIter *iter)
{
	GSQL_TRACE_FUNC;
	...
	while (gsql_cursor_fetch (cursor, 1) > 0)
	{
		...
		if (var->value_type != G_TYPE_STRING)
		{
			...
			length = g_snprintf (NULL,0, "%x%s%d%s",
				   session, name, EXCEPTION_ID, name);
		...
		gtk_tree_store_set (GTK_TREE_STORE(model), &child,
					GSQL_NAV_TREE_ID,			EXCEPTION_ID,
					GSQL_NAV_TREE_OWNER,		name,
					GSQL_NAV_TREE_IMAGE,		GSQLE_FIREBIRD_STOCK_EXCEPTIONS,
