...
engine_session_open (GtkWidget *logon_widget, gchar *buffer) {
  ...
  GSQLEPGSQLSession *pgsql_session;
  guint port = 5432;
  GSQLEPGSQLSession *test_sess = NULL;
  ...
  if (!pgsql_session_open (pgsql_session, username, password, database, hostname, port)) {
    if (buffer)
      g_strlcpy (buffer, (const gchar *) "Erro ao conectar.", 256);
  ...
  test_sess = (GSQLEPGSQLSession *) session->spec;
  GSQL_DEBUG ("Current Connection: [%p]", test_sess->pgconn);
