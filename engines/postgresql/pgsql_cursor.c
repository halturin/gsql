...
pgsql_cursor_open_bind (GSQLCursor *cursor, GList *args)
{
  ...
  while (vlist) {
	...
	paramLengths[n] = g_utf8_strlen((gchar *) vlist->data, 1048576);
	paramFormats[n] = 0;
	break;
  ...
  e_cursor->result = PQexecParams(e_session->pgconn, cursor->sql, binds_arg, paramTypes, paramValues, paramLengths, paramFormats, 0);
  ...
  GSQL_DEBUG ("BIND: [%d fields] - [%s] - [%d]", n_fields, cursor->sql, PQntuples (e_cursor->result) );
...
pgsql_cursor_open (GSQLCursor *cursor) {
  ...
  /* e_cursor->result = pgsql_stmt_result_metadata(e_cursor->statement); */
  ...
  for (n = 0; n < n_fields; n++) {
    ...
    //GSQL_DEBUG ("field[%d] = %s", n, field->name);
    var = gsql_variable_new ();
    pgsql_variable_init (var, field);
    cursor->var_list = g_list_append (cursor->var_list, var);
  }
...
pgsql_cursor_fetch (GSQLCursor *cursor, gint rows)
{
  ...
  for ( vlist = g_list_first(vlist), nvar=0; 
	...
	vlist = g_list_next(vlist), nvar++ ) 
    {
      ...
      GSQL_DEBUG ("Colum [%d] = [%s] [%d]", nvar, var->value, var->value);
    }
  ...
  return 1;
  /* if ( curr <= PQntuples(e_cursor->result) ) return 1; */
  /* return curr = 0; */
}
