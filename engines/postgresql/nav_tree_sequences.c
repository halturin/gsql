			...
			GtkTreeIter *iter)
{
	...
	//GSQL_DEBUG("DBG - Sequences: Owner [%s]", owner);
	...
	while (gsql_cursor_fetch (cursor, 1) > 0) {
		...
		if (var->value_type != G_TYPE_STRING) {
			...
			gchar *seqSQL = g_strdup_printf("select * from %s", 
							name);
			GSQL_DEBUG ("Seq: %s", seqSQL);
			det_cursor = gsql_cursor_new (session, seqSQL);
			g_free (seqSQL);

			state = gsql_cursor_open (det_cursor, FALSE);

			if (state != GSQL_CURSOR_STATE_OPEN) {
				gsql_cursor_close (det_cursor);
				return;		
			}
			...
			details = gsql_navigation_get_details (navigation,
							       key);
			pgsql_navigation_fill_details (det_cursor, details);
			gsql_cursor_close (det_cursor);
		}
