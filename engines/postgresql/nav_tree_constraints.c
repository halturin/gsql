			      ...
			      GtkTreeIter *iter)
{
	...
	GtkTreeIter child, parent, gparent, child_fake, child_last;
	GSQLCursor *cursor;
	...
	while (gsql_cursor_fetch (cursor, 1) > 0) {
		...
		if ((var->value_type != G_TYPE_STRING) || 
		    (var_t->value_type != G_TYPE_STRING))
		  {
		...
		GSQL_DEBUG("Constraint [%s]", var_t->value);

		switch (*(gchar *) var_t->value) {
			case 'p':
				GSQL_DEBUG ("Constraint type: c_type = P "\
					    "(primary key)");
				stock = GSQL_STOCK_CONSTRAINT_P;
				break;

			case 'u':
				GSQL_DEBUG ("Constraint type: c_type = U "\
					    "(unique key)");
				stock = GSQL_STOCK_CONSTRAINT_U;
				break;
			  
			default:
				GSQL_DEBUG ("Constraint type: c_type = ??? "\
					    "(wtf?)");
