			 ...
			 GtkTreeIter *iter, guint event)
{
  ...
  while (gsql_cursor_fetch (cursor, 1) > 0) {
    ...
    gtk_tree_store_set (GTK_TREE_STORE(model), &child,
			...
			GSQL_NAV_TREE_OBJECT_POPUP, nav_tree_databases_popup,
			GSQL_NAV_TREE_OBJECT_HANDLER, NULL, //FIXME: nav_tree_databases_editor,
			GSQL_NAV_TREE_EXPAND_HANDLER, NULL,
			GSQL_NAV_TREE_EVENT_HANDLER, nav_tree_databases_event,
			GSQL_NAV_TREE_STRUCT, databases,
			GSQL_NAV_TREE_DETAILS, details,
			GSQL_NAV_TREE_NUM_ITEMS, G_N_ELEMENTS(databases),
			-1);
